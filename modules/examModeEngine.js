 ...
 * Modes:     quick (20Q/30min), topic (20Q/30min), full (60Q/90min)
...
import { QUESTIONS, DOMAINS, buildQuestionSet } from '../data/examQuestions.js';
...
const MODE_CONFIG = {
  ...
  full:  { label: 'Full Exam',      count: 60, minutes: 90 },
};
...
class ExamModeEngine {
  ...
  reset() {
    ...
    if (this.container) this._showLobby();
  }
  ...
  _computeAnalytics(history) {
    ...
    return { totalExams, passRate, avgScore, streak, weakDomains };
  }
  ...
  _showLobby() {
    ...
    this.container.innerHTML = `
      <div class="module-header">
        ...
        <h1 class="module-header__title">📝 CCNA Exam Mode</h1>
      ...
      <div class="ex-analytics-strip">
        <div class="ex-analytics-card">
          <div class="ex-analytics-icon">📊</div>
      ...
      <!-- WEAK AREAS -->
      ${analytics.weakDomains.length > 0 ? `
      <div class="ex-weak-areas">
        <div class="ex-weak-title">⚠️ Areas to Focus On</div>
        <div class="ex-weak-tags">
          ${analytics.weakDomains.map(d => `
            <span class="ex-weak-tag" data-domain="${d.domain}">
      ...
      <div class="ex-mode-grid">
        ${['quick','topic','full'].map(m => `
          <div class="ex-mode-card ${this._mode===m?'active':''}" data-mode="${m}">
            <div class="ex-mode-icon">${m==='quick'?'⚡':m==='topic'?'🎯':'🏆'}</div>
  ...
  _startExam() {
    ...
    this._currentIdx = 0;
    this._screen     = 'exam';
    const totalSecs  = MODE_CONFIG[this._mode].minutes * 60;
    this._showExamShell();
  ...
  _showExamShell() {
    ...
    const cfg   = MODE_CONFIG[this._mode];

    this.container.innerHTML = `
      <!-- TOP BAR -->
      <div class="ex-topbar">
        <div class="ex-topbar-left">
          <span class="ex-topbar-title">CCNA ${cfg.label}</span>
        ...
        <div class="ex-topbar-right">
          <span class="ex-timer-ico">⏱</span>
          <span class="ex-timer" id="exam-timer">${this._fmtTime(cfg.minutes*60)}</span>
        </div>
      </div>
      ...
      <div class="ex-bottombar">
        <button class="ex-flag-btn" id="ex-flag-btn">🚩 Flag</button>
        <div style="display:flex;gap:8px;align-items:center">
          <button class="ex-nav-btn" id="ex-prev-btn">← Prev</button>
          <button class="ex-map-btn" id="ex-map-btn">📋 ${total} Qs</button>
      ...
      <div class="ex-map-overlay" id="ex-map-overlay" style="display:none">
        <div class="ex-map-panel">
          <div class="ex-map-header">
            ...
            <button class="ex-map-close" id="ex-map-close">✕</button>
          </div>
    ...
    area.innerHTML = `
      <div class="ex-q-slide-in">
        <div class="ex-q-meta">
          ...
          ${this._flagged.has(q.id) ? '<span class="ex-q-flagged">🚩 Flagged</span>' : ''}
        </div>
    ...
    return `<div class="ex-order-list" id="ex-order-list">
      ${ordered.map((id, idx) => `
        <div class="ex-order-item" draggable="true" data-id="${id}" data-idx="${idx}">
          <span class="ex-order-handle">⠿</span>
  ...
  _submitExam(timeUp) {
    this._killTimer();
    const totalSeconds = MODE_CONFIG[this._mode].minutes * 60;
  ...
  _calcScore(timeUsed) {
    ...
    this._questions.forEach(q => {
      ...
      if (q.type === 'single') {
        ...
        const given   = Array.isArray(ans) ? ans : q.items.map(i=>i.id);
        const correct = q.correct;
        let   correct_positions = 0;
        correct.forEach((id, i) => { if (given[i] === id) correct_positions++; });
        pts = correct_positions / correct.length;
        max = 1;
      }
    ...
    this.container.innerHTML = `
      <div class="module-header">
        ...
        <h1 class="module-header__title">
          ${r.passed ? '✅ Exam Passed!' : '❌ Exam Not Passed'}
        </h1>
      </div>

      ${timeUp ? `<div class="ex-timeup-banner">⏱ Time expired — exam submitted automatically</div>` : ''}
        ...
        ${Object.entries(DOMAIN_NAMES).map(([k,v]) => {
          ...
          return `
            <div class="ex-breakdown-row">
              <span class="ex-breakdown-icon">${icon}</span>
      ...
      <div class="ex-result-actions">
        <button class="ex-action-btn primary" id="ex-review-btn">📖 Review Answers</button>
        <button class="ex-action-btn secondary" id="ex-retake-btn">🔄 Retake Exam</button>
    ...
    this.container.innerHTML = `
      ...
      <div class="ex-review-body">
        ...
        <div class="ex-explanation">
          <div class="ex-explanation-title">💡 Explanation</div>
    ...
    if (q.type === 'single' || q.type === 'multi') {
      ...
      return `<div class="ex-options review">
        ${q.options.map(opt => {
          ...
          const icon = isCorrectOpt ? '✅' : wasSelected ? '❌' : '';
          return `<div class="${cls}">
            <span class="ex-option-key">${opt.id.toUpperCase()}</span>
            <span class="ex-option-text">${opt.text}</span>
            ${icon ? `<span class="rev-icon">${icon}</span>` : ''}
          </div>`;
        }).join('')}
      </div>`;
    }
      ...
      return `
        <div class="ex-review-input">
          <div class="ex-review-input-row">
            ...
            <span class="ex-review-input-val ${ok?'correct-opt':'wrong-opt'}">${given||'(no answer)'} ${ok?'✅':'❌'}</span>
          </div>
          <div class="ex-review-input-row">
            <span class="ex-review-input-lbl">Correct answer:</span>
            <span class="ex-review-input-val correct-opt">${q.answer} ✅</span>
          </div>
        </div>`;
    }
    return '';
  ...
  _injectStyles() {
    ...
    .ex-mode-icon{font-size:28px;margin-bottom:8px}
    ...
    .ex-analytics-icon{font-size:24px;line-height:1}
    ...
    .ex-weak-title{font:700 12px 'Nunito',sans-serif;color:#ffb800;margin-bottom:10px}
    ...
    .ex-q-flagged{font:600 11px 'Nunito',sans-serif;color:#ffb800}
    ...
    .ex-order-handle{font-size:16px;color:#2a3f60;cursor:grab}
    ...
    .ex-flag-btn{background:transparent;border:1.5px solid #1e2d45;color:#94a3b8;
    ...
    .ex-map-btn{background:#1a2235;color:#94a3b8;border:1px solid #1e2d45;
    ...
    .ex-map-close{background:transparent;border:none;color:#94a3b8;font-size:16px;
    ...
    /* ── Results ── */
    .ex-timeup-banner{background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);
    ...
    .ex-breakdown-icon{font-size:16px;flex-shrink:0}
    ...
    .ex-action-btn{font:700 13px 'Syne',sans-serif;padding:10px 22px;border-radius:8px;
    ...
    .rev-icon{margin-left:auto;flex-shrink:0}
    ...
    .ex-explanation-title{font:700 12px 'JetBrains Mono',monospace;color:#00d4ff;
    ...
    .ex-review-input-val{font:600 14px 'JetBrains Mono',monospace;padding:6px 12px;border-radius:6px;border:1px solid #1e2d45}
